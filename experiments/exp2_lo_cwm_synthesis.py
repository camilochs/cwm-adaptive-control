#!/usr/bin/env python3
"""
Experiment 2: LeadingOnes CWM Synthesis

Synthesises a Code World Model for the (1+1)-RLS_k on LeadingOnes using an
LLM.  The CWM must implement predict_next_state, get_legal_actions,
evaluate_state, and is_terminal — but with *LOState / LOAction* instead of the
continuous-optimisation CWMState/CWMAction.

Pipeline:
  1. Load trajectories produced by exp1.
  2. Format representative samples + statistics for the prompt.
  3. Call LLM (via LLMClient) with a LeadingOnes-specific system prompt.
  4. Extract code, validate with LOState/LOAction tests.
  5. If invalid, refine (up to max_attempts).
  6. Save to results/cwm/leadingones_cwm.py.
"""
...
LO_SYSTEM_PROMPT = """\
You are synthesizing a Code World Model (CWM) for the LeadingOnes problem.

The (1+1)-RLS_k algorithm flips exactly k randomly chosen bits in a bitstring
of length n.  The offspring replaces the parent if its LeadingOnes fitness
(count of uninterrupted leading 1s from position 0) is >= the parent's.

Your CWM must implement four methods that operate on LOState and LOAction
dataclasses (already defined in the execution namespace):

  @dataclass
  class LOState:
      fitness: int          # current LO value  (0 .. n)
      n: int                # bitstring length
      step: int             # current step counter
      budget: int           # max allowed steps
      normalized_fitness: float   # fitness / n
...
Methods to implement inside class SynthesizedCWM:
  ...
  predict_next_state(state: LOState, action: LOAction) -> LOState
      Given current fitness i and flip count k, predict the expected next
      fitness and updated step count.

  get_legal_actions(state: LOState) -> List[LOAction]
      Return a list of reasonable flip-count actions for the current state.
      Must return LOAction objects.

  evaluate_state(state: LOState) -> float
      Score state quality (higher = better).

  is_terminal(state: LOState) -> bool
      True when the optimum is reached or the budget is exhausted.

Key insight: The probability of improving fitness from i to i+1 in a single
step with k-bit flips is approximately:
    P(improve | i, k) ≈ (k/n) * ((n-k) / (n-1))^i     for small k
More precisely: bit i+1 must be among the k flipped, AND none of the first i
bits may be flipped.

The theoretically optimal policy is  k*(i) = floor(n / (i + 1)).
...
- The normalized_fitness field must store the CONTINUOUS expected fitness
  divided by n, i.e.  (i + P(improve|i,k)) / n.  This allows evaluate_state
...
LO_SYNTHESIS_PROMPT = """\
Based on the following trajectory data from (1+1)-RLS_k on LeadingOnes,
synthesize a CWM.
...
outside the standard library; math is available).  LOState and LOAction are
...
## Reminders
- LOState and LOAction are already defined — do NOT redefine them.
- predict_next_state must return an LOState with step = state.step + 1.
- evaluate_state must return a finite float (no NaN / Inf).
- get_legal_actions must return a non-empty list of LOAction objects.
...
# ── LOState / LOAction validation ───────────────────────────────────────────

def _make_namespace():
    """Create an execution namespace with LOState and LOAction defined."""
    setup = """\
...
def validate_lo_cwm(code: str, n: int = 50, budget: int = 2000):
    """Validate a synthesised LeadingOnes CWM.
    ...
    LOState = ns["LOState"]
    LOAction = ns["LOAction"]

    # Test state & action
    state = LOState(fitness=10, n=n, step=50, budget=budget,
                    normalized_fitness=10 / n)
    action = LOAction(k=5, name="flip_5")

    # predict_next_state
    try:
        ns_ = cwm.predict_next_state(state, action)
        if not isinstance(ns_, LOState):
            errors.append("predict_next_state: did not return LOState")
    ...
    try:
        ...
        elif not all(isinstance(a, LOAction) for a in actions):
            errors.append("get_legal_actions: elements are not LOAction")
        else:
    ...
    edge = LOState(fitness=0, n=n, step=0, budget=budget, normalized_fitness=0.0)
    edge_action = LOAction(k=n, name=f"flip_{n}")
    try:
...
def synthesize(data, model: str, max_attempts: int = 5, verbose: bool = True):
    """Synthesise + validate a LeadingOnes CWM.
...
def main():
    ...
    print("=" * 60)
    print("Experiment 2: LeadingOnes CWM Synthesis")
    print("=" * 60)
    ...
    if args.validate_only:
        ...
        with open(args.output) as f:
            code = f.read()
        valid, tests, errors = validate_lo_cwm(code)
        for tname, ok in tests.items():
    ...
    # Final validation
    print("\nFinal validation:")
    valid, tests, errors = validate_lo_cwm(code, n=data["parameters"]["n"])
    for tname, ok in tests.items():
        print(f"  {tname}: {'OK' if ok else 'FAIL'}")
    print(f"\nValid: {valid}")
