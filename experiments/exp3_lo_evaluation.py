#!/usr/bin/env python3
"""
Experiment 3: LeadingOnes CWM Evaluation
...
Four sub-experiments:
  ...
  B. Policy Comparison – 6 policies (static_1, static_ln, random, optimal,
  ...
  C. Interpretability – extract the CWM's learned policy and plot vs the
     theoretical optimum k*(i) = floor(n/(i+1)).
  D. Generalisation – train on n=50, evaluate on n=100 and n=200.

Outputs:
  results/exp3/exp3_results.json        — all numerical data
  results/exp3/fig_policy_comparison.png  — learned vs optimal k(i)
  results/exp3/fig_heatmap.png            — CWM score heatmap
  results/exp3/fig_convergence.png        — convergence curves
  results/exp3/fig_generalisation.png     — generalisation bar chart
"""
...
# ── LOState / LOAction (same as in exp2) ───────────────────────────────────

def _make_lo_namespace():
    """Execution namespace with LOState & LOAction."""
    setup = """\
from dataclasses import dataclass
...
def load_lo_cwm(cwm_path: str):
    """Load a synthesised LeadingOnes CWM.

    Returns (cwm_instance, namespace) so callers can create LOState/LOAction.
    """
    with open(cwm_path) as f:
        code = f.read()
    ns = _make_lo_namespace()
    exec(code, ns)
...
def _lo_state(ns, fitness: int, n: int, step: int, budget: int):
    return ns["LOState"](
...
def _lo_action(ns, k: int):
    return ns["LOAction"](k=k, name=f"flip_{k}")
...
def evaluate_cwm_quality(
    ...
    for fit in fitness_snapshots:
        ...
        # Build a canonical bitstring with exactly `fit` leading ones
        base_x = np.zeros(n, dtype=int)
        base_x[:fit] = 1  # first `fit` bits = 1, rest = 0
        ...
        for k in test_ks:
            ...
            for trial in range(n_trials):
                ...
                improvements.append(new_fit - fit)
            gt_scores.append(float(np.mean(improvements)))

        # CWM ranking
        state = _lo_state(ns, fit, n, step=0, budget=budget)
        cwm_scores = []
        for k in test_ks:
            action = _lo_action(ns, k)
            try:
        ...
        if verbose:
            print(f"  fitness={fit:3d}  tau={tau:.3f}  gt_best_k={gt_best_k}  "
                  f"cwm_best_k={cwm_best_k}  optimal_k={optimal}")
...
def _cwm_greedy_policy(cwm, ns, n, budget):
    ...
    candidate_ks = [max(1, n // (i + 1)) for i in range(n)]
    # Use a representative set of k values
    candidate_ks = sorted(set([1, 2, 3, 5, 8, 10, 15, 25]
                               + [max(1, n // (i + 1)) for i in range(0, n, 5)]
                               + [n]))
    candidate_ks = [k for k in candidate_ks if 1 <= k <= n]

    def policy(fitness, n_, step):
        state = _lo_state(ns, fitness, n_, step, budget)
...
def _cwm_mcts_policy(cwm, ns, n, budget, simulations=50, horizon=5):
    ...
    # Lightweight inline MCTS (avoids importing the MCTS planner which uses
    # CWMState/CWMAction instead of LOState/LOAction).
    candidate_ks = sorted(set([1, 2, 3, 5, 8, 10, 15, 25]
                               + [max(1, n // (i + 1)) for i in range(0, n, 5)]
                               + [n]))
    candidate_ks = [k for k in candidate_ks if 1 <= k <= n]
    ...
    def policy(fitness, n_, step):
        root_state = _lo_state(ns, fitness, n_, step, budget)
...
def run_policy_comparison(
    ...
    stateless_policies = {
        ...
        "optimal":    lambda f, n_, s: optimal_k(f, n_),
...
def compute_policy_statistics(results: Dict[str, list]):
    ...
    for i in range(len(names)):
        for j in range(i + 1, len(names)):
            a, b = np.array(results[names[i]], dtype=float), np.array(results[names[j]], dtype=float)
            try:
...
def extract_learned_policy(cwm, ns, n: int = 50, budget: int = 2000):
...
def build_heatmap_data(cwm, ns, n: int = 50, budget: int = 2000):
    """Build fitness × k score matrix for heatmap visualisation."""
...
def run_generalisation(
    ...
    for n in sizes:
        ...
        stateless_pols = [("optimal", optimal_fn), ("CWM-greedy", greedy_fn),
                          ("RLS_1", static1_fn)]
        stateful_pols = [("fifth_rule", lambda: _ea_alpha_policy(n)),
                         ("self_adjusting", lambda: _self_adjusting_policy(n))]
        ...
        for pname, factory in stateful_pols:
            ...
            for ep in range(n_episodes):
                seed = ep * 37 + n
                episode = run_episode(n, factory(), budget, seed=seed)
                steps_list.append(episode["total_steps"])
...
def plot_policy_comparison(learned_policy, n, plt, path):
    """Fig 1: CWM learned k(i) vs optimal k*(i)."""
    _pub_style(plt)
    ...
    ax.plot(fitnesses, opt_ks, color=_C["theory"], ls="-", lw=2.5,
            label=r"Optimal $k^*(i) = \lfloor n/(i\!+\!1) \rfloor$")
...
def plot_heatmap(heatmap_data, plt, path):
    ...
    # Row-normalise: z-score per fitness level so the *relative* ranking shows
    ...
    opt_line_y = []
    for fit in fitnesses:
        ok = optimal_k(fit, fitnesses[-1] + 1)  # approximate n
        if ok in ks:
    ...
    ax.set_ylabel("LeadingOnes fitness $i$")
...
def plot_generalisation(gen_results, plt, path):
    """Fig 4: normalised steps (T / n^2) across problem sizes."""
    _pub_style(plt)
    ...
    for i, (pol, lab, col) in enumerate(zip(pol_order, pol_labels, pol_colors)):
        ...
        for sz in sizes:
            ...
            if key in gen_results:
                means.append(gen_results[key]["mean_steps"] / (sz * sz))
                stds.append(gen_results[key]["std_steps"] / (sz * sz))
            else:
                means.append(0)
                stds.append(0)
    ...
    ax.set_ylabel(r"Normalised steps  $T\,/\,n^2$")
    ax.legend(framealpha=0.9)

    # Reference lines for theoretical bounds
    ax.axhline(0.5, color=_C["RLS_1"], ls=":", lw=1, alpha=0.5)
    ax.axhline(0.39, color=_C["optimal"], ls=":", lw=1, alpha=0.5)
    ax.text(x[-1] + 2.2 * width, 0.50, r"$\approx 0.5n^2$", fontsize=9,
            color=_C["RLS_1"], va="bottom")
    ax.text(x[-1] + 2.2 * width, 0.39, r"$\approx 0.39n^2$", fontsize=9,
            color=_C["optimal"], va="top")
...
def run_experiment(args):
    ...
    print("=" * 60)
    print("Experiment 3: LeadingOnes CWM Evaluation")
    print("=" * 60)
    print(f"  n={n}, budget={budget}, episodes={n_episodes}")

    # Load CWM
    print(f"\nLoading CWM from: {args.cwm}")
    cwm, ns = load_lo_cwm(args.cwm)

    all_results: Dict[str, Any] = {
        "experiment": "exp3_lo_evaluation",
...
def main():
    parser = argparse.ArgumentParser(
        description="Exp30: LeadingOnes CWM full evaluation")
    parser.add_argument("--cwm", type=str,
                        default="results/cwm/leadingones_cwm.py",
    ...
    parser.add_argument("--budget", type=int, default=None,
                        help="Step budget (default 0.8*n^2)")
