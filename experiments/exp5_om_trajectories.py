#!/usr/bin/env python3
"""
Experiment 5: OneMax Trajectory Collection

Collects (1+1)-RLS_k trajectories on the OneMax problem using six
...
Policies:
  ...
  5. adaptive     – k = max(1, n - i)  (flip ~all zeros)
  6. increasing   – k = max(1, fitness + 1)

Output: results/trajectories/om_trajectories.json
"""
...
def policy_adaptive(fitness: int, n: int, step: int) -> int:
    """Flip approximately all zeros: k = max(1, n - i)."""
    return max(1, n - fitness)


def policy_increasing(fitness: int, n: int, step: int) -> int:
    """Grow k as fitness grows: k = max(1, fitness + 1)."""
    return max(1, fitness + 1)
...
def main():
    ...
    print("=" * 60)
    print("Experiment 5: OneMax Trajectory Collection")
    print("=" * 60)
    print(f"  n            = {n}")
    ...
    data = {
        "version": "1.0",
        "experiment": "exp5_om_trajectories",
